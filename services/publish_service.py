...
def make_release_zips(font_config):
    for size_mode in configs.size_modes:
        for font_format in ['otf', 'woff2', 'ttf']:
            zip_file_output_path = os.path.join(workspace_define.releases_dir, font_config.get_release_zip_file_name(size_mode, font_format))
            with zipfile.ZipFile(zip_file_output_path, 'w') as zip_file:
                for locale_flavor in configs.locale_flavors:
                    font_file_name = font_config.get_output_font_file_name(size_mode, locale_flavor, font_format)
...
def copy_docs_files(font_config):
    for size_mode in configs.size_modes:
        for locale_flavor in configs.locale_flavors:
            woff2_file_name = font_config.get_output_font_file_name(size_mode, locale_flavor, 'woff2')
    ...
    ext_file_names = [
        font_config.get_ext_file_name('font-info', 'md'),
        font_config.get_ext_file_name('preview', 'png'),
        font_config.get_ext_file_name('alphabet', 'html'),
        font_config.get_ext_file_name('demo', 'html')
    ]
